...
namespace weapon_bts_broom
{
	...
	class weapon_bts_broom : ScriptBasePlayerWeaponEntity, bts_rc_base_weapon, bts_rc_base_melee
	{
		...
		void Spawn()
		{
			g_EntityFuncs.SetModel(self, self.GetW_Model("models/bts_rc/weapons/w_broom.mdl"));
		...
		bool Deploy()
		{
			return bts_deploy("models/bts_rc/weapons/v_broom.mdl", "models/bts_rc/weapons/p_broom.mdl", DRAW, "crowbar", 1);
		}
		...
		private bool Swing(bool fFirst)
		{
			...
			if (tr.flFraction >= 1.0f)
			{
				if (fFirst)
				{
					...
					self.m_flNextPrimaryAttack = g_Engine.time + (is_trained_personal ? 0.52f : 0.67f);
			...
			else
			{
				...
				self.m_flNextPrimaryAttack = g_Engine.time + (is_trained_personal ? 0.26f : 0.42f);
				self.m_flTimeWeaponIdle = g_Engine.time + 2.0f;
				...
				if (fHitWorld)
				{
					...
					// also play broom strike
					switch (Math.RandomLong(1, 2))
					{
						case 2:
							g_SoundSystem.EmitSoundDyn(m_pPlayer.edict(), CHAN_WEAPON, "debris/wood2.wav", 1.0f, ATTN_NORM, 0, 92 + Math.RandomLong(0, 3));
							break;
						default:
							g_SoundSystem.EmitSoundDyn(m_pPlayer.edict(), CHAN_WEAPON, "debris/wood1.wav", 1.0f, ATTN_NORM, 0, 92 + Math.RandomLong(0, 3));
							break;
					}
				}
