 ...
 * Spanner/Wrench
 */
// Idea: SV BOY, RaptorSKA
// Revisited by AraseFiq._

namespace weapon_bts_spanner
{
	enum spanner_e
	{
		IDLE = 0,
	...
	class weapon_bts_spanner : ScriptBasePlayerWeaponEntity, bts_rc_base_weapon, bts_rc_base_melee
	{
		...
		void Spawn()
		{
			g_EntityFuncs.SetModel(self, self.GetW_Model("models/bts_rc/weapons/w_spanner.mdl"));
		...
		bool Deploy()
		{
			return bts_deploy("models/bts_rc/weapons/v_spanner.mdl", "models/bts_rc/weapons/p_spanner.mdl", DRAW, "crowbar", 1);
		}
		...
		private bool Swing(bool fFirst)
		{
			...
			if (tr.flFraction >= 1.0f)
			{
				if (fFirst)
				{
					...
					{
						case 0:
							self.SendWeaponAnim(ATTACK1, 0, pev.body);
							break;
						case 1:
							self.SendWeaponAnim(ATTACK2, 0, pev.body);
							break;
						case 2:
							self.SendWeaponAnim(ATTACK1, 0, pev.body);
							break;
					}
					self.m_flNextPrimaryAttack = g_Engine.time + (is_trained_personal ? 0.3f : 0.4f);
			...
			else
			{
				...
				self.m_flNextPrimaryAttack = g_Engine.time + (is_trained_personal ? 0.2f : 0.3f);
				self.m_flTimeWeaponIdle = g_Engine.time + 2.0f;
				...
				if (fHitWorld)
				{
					...
					// also play spanner strike
					switch (Math.RandomLong(1, 2))
					{
						case 2:
							g_SoundSystem.EmitSoundDyn(m_pPlayer.edict(), CHAN_WEAPON, "weapons/cbar_hit2.wav", 1.0f, ATTN_NORM, 0, 105 + Math.RandomLong(0, 3));
							break;
						default:
							g_SoundSystem.EmitSoundDyn(m_pPlayer.edict(), CHAN_WEAPON, "weapons/cbar_hit1.wav", 1.0f, ATTN_NORM, 0, 105 + Math.RandomLong(0, 3));
							break;
					}
				}
